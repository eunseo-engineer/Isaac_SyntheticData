...
class Extension(omni.ext.IExt):
                ...
                for viewport_name in self._viewport_names:
                    ...
                    with ui.CollapsableFrame(viewport_name + ": Sensor Settings", height=0):
                        with ui.VStack(spacing=5):
                            ...
                            def toggle_pose(self, viewport_name, value):
                                self._sensor_settings[viewport_name]["pose"]["enabled"] = value
                            ...
                            with ui.HStack(height=30):
                                ui.Spacer(width=10)
                                ui.Label("Pose", height=0, width=150)
                                self._sensor_settings_ui[viewport_name]["pose"]["checkbox"] = ui.CheckBox()
                                self._sensor_settings_ui[viewport_name]["pose"]["checkbox"].model.add_value_changed_fn(
                                    lambda a, v=viewport_name, this=self: toggle_pose(
                                        self, v, a.get_value_as_bool()
                                    )
                                )
                            with ui.HStack(height=30):
    ...
    def _update(self, e: carb.events.IEvent):
        ...
        for viewport_name in self._viewport_names:
            ...
            if self._sensor_settings[viewport_name]["pose"]["enabled"]:
                 gt_list.append("pose")  
